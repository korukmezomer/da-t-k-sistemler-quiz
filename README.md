...
| DB            | localhost:5433 / shopwave               |

## LAB-2: Latency ve Jitter

Backend içinde kontrollü yapay gecikme artık açılıp kapatılabilir.

Örnek:

```bash
export SHOPWAVE_CHAOS_DELAY_ENABLED=true
export SHOPWAVE_CHAOS_DELAY_FIXED_MS=200
export SHOPWAVE_CHAOS_DELAY_JITTER_MS=300
export SHOPWAVE_CHAOS_DELAY_TARGET_OPERATIONS=placeOrder,reserveStock
docker compose up --build
```

Bu ayarla:
- `placeOrder` her çağrıda en az `200ms` bekler
